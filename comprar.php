<html>
	...
	<style>
        ...
        <section id="background">
            ...
            <div id="scroll";>
            <h1 id="mensaje1"> Gracias por hacer su compra en nuestra tienda! </h1>
            <?php
            date_default_timezone_set("America/Tijuana");

            $total = $_POST['total'];
            $fechaPago = date("Y-m-d h:i:sa");
            $estatus = false;

            $sql = "SELECT * FROM ventas";
            $result = $conn->query($sql);

        // Itera a través de cada producto en el carrito
        while($row = $result->fetch_assoc()) {
            // Obtén el ID del producto y la cantidad comprada
            $productoID = $row['Producto'];
            $cantidadComprada = $row['Cantidad'];

            // Obtén la cantidad actual en inventario
            $sql2 = "SELECT Existencias FROM inventario WHERE No_Producto = $productoID";
            $result2 = $conn->query($sql2);
            $row2 = $result2->fetch_assoc();
            $cantidadActual = $row2['Existencias'];

            // Calcula la nueva cantidad
            $nuevaCantidad = $cantidadActual - $cantidadComprada;

            // Actualiza la cantidad en la base de datos
            $sql3 = "UPDATE inventario SET Existencias = $nuevaCantidad WHERE No_Producto = $productoID";
		
		
		$sql4 = "DELETE FROM ventas WHERE Cliente = '" . $usu . "'";
		 $result4 = $conn->query($sql4);
    
                ?>
                <?php
                    if ($conn->query($sql3) === TRUE) {
                        echo "Inventario actualizado correctamente para el producto $productoID <br>";

                        $estatus = true;
                    } else {
                        echo "Error al actualizar el inventario para el producto $productoID <br>: " . $conn->error;
                    }
                }

                $ventaCaja = "INSERT INTO caja (TotalVenta, Estatus, FechaPago) 
                VALUES ('$total', '$estatus', '$fechaPago')";
                $resultVentaCaja = $conn->query($ventaCaja);
                ?>
                <br>
