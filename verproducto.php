	...
	<style>
        ...
        <section id="background">
            ...
            <?php
                ...
                $sql = "SELECT * FROM inventario WHERE No_Producto =".$idProducto."";
                $result = $conn->query($sql);

                $sqlOnCarrito = "SELECT COUNT(*) as count FROM ventas WHERE Producto = " . $idProducto . " AND Cliente = '" . $usu . "'";
                $resultOnCarrito = $conn->query($sqlOnCarrito);
                $OnCarrito = 0;
                
                if ($resultOnCarrito->num_rows > 0) {
                    $rowExistencia = $resultOnCarrito->fetch_assoc();
                    $OnCarrito = $rowExistencia["count"];
                ...
                <tr>
                    <td id="imgProducto"> 
                        <img alt="" width="315px" height="315px" id="contenedor" src="data:image/jpg;base64,<?php echo base64_encode($show['Imagen']); ?>">
                    </td>
                    <td id="infoProducto"> 
                        <div id="info">
                        <h1 id="nombreProducto"> ➤ <?php echo $show['Nombre_Producto']; ?> </h1>
                        <h1 id="precioProducto"> $<?php echo $show['Precio_Venta']; ?> </h1>
                        <h2 id="descProducto"> <?php echo $show['Descripcion_Producto']; ?> </h2>
                        <?php echo $show['Existencias']; ?> en existencia <br> <br>

                        <form action="agregarCarrito.php?user=<?php echo $usu;?>" method="POST">
                        Unidades: <input type="number" id="unidades" name="unidades" value="1" min="1" max="<?php echo $show['Existencias']; ?>"> <br>

                        <button type="submit" id="addCarrito" name="producto" value="<?php echo $show['No_Producto']; ?>" <?php echo ($OnCarrito > 0) ? "disabled" : ""; ?>>
                        <?php echo ($OnCarrito > 0) ? "Añadido 🛒" : "Añadir 🛒"; ?> </button>
                        </form>
                        </div>
                    </td>
                    </td>
